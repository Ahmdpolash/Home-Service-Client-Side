...
const Other = ({ otherService }) => {
    const {description} = otherService
     ...
     <div className="relative flex flex-col  w-full mt-8 border lg:flex-row rounded-xl bg-white bg-clip-border text-gray-700 shadow-lg">
        ...
        <div className="px-6 py-2">
          ...
          <img
            ...
            src={otherService.provider_image}
            alt=""
          />
          <div className="flex flex-row  justify-between items-center py-2 mt-2">
            <div>
              <h2 className="text-[18px] font-bold">{otherService.provider_name}</h2>
            </div>
            <div>
              <Link to={`/details/${otherService._id}`}>
                <button
