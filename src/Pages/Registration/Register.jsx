...
const Register = () => {
  ...
  const handleRegister = (e) => {
    ...
    if (password.length < 6) {
      return toast.error("Password must be at least 6 characters long",{ id: toastId });
    } else if (!/[A-Z]/.test(password)) {
      return toast.error("Password must contain at least one Capital letter.",{ id: toastId });
    } else if (!/[!@#$%^&*:;?,.]/.test(password)) {
      return toast.error(
        "Password must contain at least one special character",{ id: toastId }
      );
    }
    ...
    <div className="h-[90vh] mb-4 mt-2 flex items-center justify-center ">
      ...
      >
        ...
        <input
          ...
          required
        />
        <span onClick={() => setShow(!show)} className="absolute right-9 mt-6">
          {show ? (
